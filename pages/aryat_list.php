...
<section class="content-header">
  <h1>
    งานยึดทรัพย์และอายัด
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><span class="active">งานยึดทรัพย์และอายัด</span></a></li>
...
<div class="row">
    <div class="col-xs-12">
    <button type="button" class="btn btn-app flat" onClick="goHref('../services/insert.php?type=add_aryat')" title="new">
    ...
    <div class="box">
        ...
        <div class="box-body">
          <table id="example1" class="table table-bordered table-hover table-fixed">
            ...
            <tbody>
<?php 	$index = 1;
    $strSQL = "SELECT * FROM `document_notis` LEFT JOIN `plaintiff` ON document_notis.doc_plaintiff_id = plaintiff.plaintiff_id LEFT JOIN `lawyer` ON document_notis.lawyer_id = lawyer.lawyer_id WHERE `document_notis`.`doc_type` = 3";
    $objQuery = $conDB->sqlQuery($strSQL);
	while($objResult = mysqli_fetch_assoc($objQuery)) { ;?>
            <tr onDblClick="goHref('../pages/aryat_edit.php?act=view&id=<?php echo $objResult['doc_id'] ?>')">
              <td><?php echo $index++; ?></td>
              <td align="center" style="font-size:16px;">
              <i class="fa fa-search text-blue" onClick="goHref('../pages/aryat_edit.php?act=view&id=<?php echo $objResult['doc_id'] ?>')" title="view"></i>
              <i class="fa fa-pencil text-yellow" onClick="goHref('../pages/aryat_edit.php?act=edit&id=<?php echo $objResult['doc_id'] ?>')" title="edit"></i>
