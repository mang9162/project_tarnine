...
if($type == 'add_notis'){
    ...
    $strSQL = "INSERT INTO `$table_name` (`doc_id`, `doc_no`, `doc_type`, `doc_create_date`, `doc_plaintiff_id`, `doc_plaintiff_name`, `doc_restructuring`, `doc_credit_type`, `doc_county`, `doc_notis_date`, `lawyer_id`, `lawyer_name`, `doc_status`, `create_by_user`) VALUES (NULL, '',1, CURRENT_TIMESTAMP, '', '', '', '', '', NULL, '', '', 'รอ', '$username');";
...
if($type == 'add_seupsub'){
    // $doc_id = $conDB->sqlEscapestr($_GET['doc_id']);
    $table_name = 'document_notis';
    $strSQL = "INSERT INTO `$table_name` (`doc_id`, `doc_no`, `doc_type`, `doc_create_date`, `doc_plaintiff_id`, `doc_plaintiff_name`, `doc_restructuring`, `doc_credit_type`, `doc_county`, `doc_notis_date`, `lawyer_id`, `lawyer_name`, `doc_status`, `create_by_user`) VALUES (NULL, '',2, CURRENT_TIMESTAMP, '', '', '', '', '', NULL, '', '', 'รอ', '$username');";
    $objQuery = $conDB->sqlQuery($strSQL);

    $strSQL = "SELECT * FROM `$table_name` ORDER BY `document_notis`.`doc_create_date` DESC LIMIT 1";
    $objQuery = $conDB->sqlQuery($strSQL);
    while($objResult = mysqli_fetch_assoc($objQuery)) {
        $redirect = "<script>window.location.href = '../pages/seupsub_edit.php?id=".$objResult['doc_id']."&act=edit'</script>";
    }
}
...
if($type == 'add_aryat'){
    // $doc_id = $conDB->sqlEscapestr($_GET['doc_id']);
    $table_name = 'document_notis';
    $strSQL = "INSERT INTO `$table_name` (`doc_id`, `doc_no`, `doc_type`, `doc_create_date`, `doc_plaintiff_id`, `doc_plaintiff_name`, `doc_restructuring`, `doc_credit_type`, `doc_county`, `doc_notis_date`, `lawyer_id`, `lawyer_name`, `doc_status`, `create_by_user`) VALUES (NULL, '',3, CURRENT_TIMESTAMP, '', '', '', '', '', NULL, '', '', 'รอ', '$username');";
    $objQuery = $conDB->sqlQuery($strSQL);

    $strSQL = "SELECT * FROM `$table_name` ORDER BY `document_notis`.`doc_create_date` DESC LIMIT 1";
    $objQuery = $conDB->sqlQuery($strSQL);
    while($objResult = mysqli_fetch_assoc($objQuery)) {
        $redirect = "<script>window.location.href = '../pages/aryat_edit.php?id=".$objResult['doc_id']."&act=edit'</script>";
    }
}
